...
### Project Constraints
1. **Archive Type**: CBR and CBZ files in this project are zip files (using `.cbr` or `.cbz` extensions) containing image files. Use the built-in `zipfile` module. Do not require native `unrar` libraries.
2. **Page Formats**: Extract page files directly without transcoding, preserving their original format and extension (supporting `.webp`, `.jpg`, `.jpeg`, `.png`). Only convert the cover image page to JPEG format (`cover.jpg`).
