# Spec: CBR/CBZ Book Parser

## Objective
The program will parse a set of `.cbr` or `.cbz` (ZIP archives containing comic pages in formats like WebP, JPG, JPEG, and PNG) files in a source directory, group pages by chapter, and organize them into an output structure where each archive file gets its own directory named after the book filename (excluding the extension).

Specifically, it will take files from:
`source/[filename].cbr` or `source/[filename].cbz`
...
├── source/                  # Input CBR/CBZ files (provided)
...
## Success Criteria
1. The parser processes all `.cbr` and `.cbz` files in the source directory.
